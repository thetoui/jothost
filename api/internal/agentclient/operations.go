...
// GrafanaStatus is what the host says about Grafana.
...
// GrafanaSetup is what the Agent needs to provision it.
//
// The panel supplies its own database's details because the Agent has no
// reason to know them and no way to find them out. The password is the
// read-only role's, generated by the API for this purpose.
type GrafanaSetup struct {
...
// payload turns the setup into what the socket carries.
//
// Written out rather than marshalled through a helper, so the field names the
// Agent decodes are visible next to the values that fill them.
func (s GrafanaSetup) payload() map[string]any {
	return map[string]any{
		...
		"db_sslmode":  s.SSLMode,
		"root_url":    s.RootURL,
	}
}

// Grafana reports what is on the host.
func (c *Client) Grafana(ctx context.Context, requestID string) (GrafanaStatus, error) {
	var status GrafanaStatus
	err := c.call(ctx, requestID, protocol.OperationGrafanaStatus, nil, &status)
	return status, err
}

// InstallGrafana installs and provisions it.
//
// Submitted as a background job by the caller: this is a package download of
// several hundred megabytes on some distributions, and the panel's request
// timeout is measured in seconds.
func (c *Client) InstallGrafana(ctx context.Context, requestID string, setup GrafanaSetup) (string, error) {
	return c.SubmitAsync(ctx, requestID, protocol.OperationGrafanaInstall, setup.payload())
}

// ProvisionGrafana rewrites the panel's files without installing.
func (c *Client) ProvisionGrafana(ctx context.Context, requestID string, setup GrafanaSetup) (GrafanaStatus, error) {
	var status GrafanaStatus
	err := c.call(ctx, requestID, protocol.OperationGrafanaProvision, setup.payload(), &status)
	return status, err
}
